...
class SearchEngineManager:
    ...
    def search_products(self, query: str, retriever_type: str = "faiss", top_k: int = 30):
        """상품 검색 수행"""
        try:
            ...
            # 검색 수행
            if retriever_type == "faiss":
                # FAISS의 경우 similarity_search_with_score 사용
                results = self.faiss_db.similarity_search_with_score(query, k=top_k)
                
                # 결과 변환
                products = []
                for doc, score in results:
                    goods_no = doc.metadata.get("goodsNo", "unknown")
                    # FAISS의 거리 점수를 유사도 점수로 변환 (거리가 작을수록 유사도가 높음)
                ...
                for i, doc in enumerate(docs[:top_k]):
                    goods_no = doc.metadata.get("goodsNo", f"unknown_{i}")
                    # BM25는 점수 정보가 없으므로 순위 기반 점수 계산
                    similarity_score = max(0, 1 - (i / len(docs)))
                    products.append({
                        "goodsNo": goods_no,
                        "similarity_score": similarity_score,
                        "content": doc.page_content[:100] + "..." if len(doc.page_content) > 100 else doc.page_content
            ...
            logger.info(f"검색 완료: 쿼리='{query}', 검색기='{retriever_type}', 결과={len(products)}개")
            return products
